...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE fn_asset_inv SET jumlah=%s, foto=%s, lokasi=%s, no_inventaris=%s, keterangan=%s, tgl_update=%s WHERE id_material=%s",
  ...
  $Result1 = mysql_query($updateSQL, $core) or die(mysql_error());
    ...
    echo "<script>
  	alert(\"Data Asset has been updated\");
	self.close();
...
mysql_select_db($database_core, $core);
$query_Recordset1 = sprintf("SELECT fn_asset_inv.*, m_master.item_code, m_master.descr_name, m_master.descr_spec, m_master.id_type, m_master.id_unit, m_master.brand, m_e_model.mtrl_model, m_unit.unit FROM fn_asset_inv, m_master, m_e_model, m_unit WHERE fn_asset_inv.id = %s AND m_master.id_item = fn_asset_inv.id_material AND m_master.id_mmodel = m_e_model.id_mmodel AND m_master.id_unit = m_unit.id_unit", GetSQLValueString($colname_Recordset1, "int"));
...
<form method="POST" action="<?php echo $editFormAction; ?>" name="form1" id="form1">
  <table width="383">
    ...
    <tr>
      <td><input type="text" name="id_material" id="id_material" value="<?php echo $row_Recordset1['id_material']; ?>" /></td>
