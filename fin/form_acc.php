  ...
  <tr>
    <td width="950">
      ...
      <p>
		...
		$sql=mysql_query("SELECT * FROM a_crf WHERE (a_crf.approval='Y') AND (a_crf.projectcode LIKE '%$cari%') AND (a_crf.acc=1) ORDER BY nocrf ASC ");
	  $totalseluruhpendaftar=mysql_num_rows($sql);
	  if($totalseluruhpendaftar=='0'){
	 echo "<center><blink>No data available in table </bink></center>";
	}
...
while($sql2=mysql_fetch_array($not)){
	  ?>
          <th colspan="4"><?php echo $sql2[project_code] . " - " . $sql2[customer];?>            <? } ?></th>
        </tr>
        <tr class="tabel_header">
          ...
          <th width='126' class="tabel_header">Payments</th>
          <th width='115' class="tabel_header">Invoicing</th>
        </tr>
        <?
while($sql1=mysql_fetch_array($sql)){
	  ?>
        <tr class="tabel_body">
          ...
          <td><?php echo $sql1[projectcode] . " - " .$sql1[productioncode];?></td>
          <td ><a href="work_fin.php?data=<?php echo $sql1['nocrf'];?>" target="_blank"><b><?php echo $sql1[jobtitle];?></b></a></td>
