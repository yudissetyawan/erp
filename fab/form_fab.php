...
$query_Recordset1 = sprintf("SELECT * FROM a_proj_code WHERE complete = %s ORDER BY project_code ASC", GetSQLValueString($colname_Recordset1, "text"));
...
?>
	  ...
	  ?>
        <tr class="tabel_body">
          ...
          <td ><a href="#"><b onClick="MM_openBrWindow('../prj/viewcrfdetail.php?data=<?php echo $sql1[nocrf];?>','crf','width=1200,height=500')"> <?php echo $sql1[jobtitle] ?>
</b></a></td>
             <td align='center'><?php 
		if ($sql1[marketing]==1) 
		{
  ...
  <tr>
    <td width="950">
      ...
      <p>
		...
		$sql=mysql_query("SELECT * FROM a_crf WHERE (a_crf.approval='Y') AND (a_crf.projectcode LIKE '%".$cari."%') AND (a_crf.fabrication=1) ORDER BY nocrf ASC ");
	  $totalseluruhpendaftar=mysql_num_rows($sql);
	  if($totalseluruhpendaftar=='0'){
	 echo "<center><blink>No data available in table </bink></center>";
	}
	  ...
	  ?>
      <table width="1162" border="0" cellspacing="1" cellpadding="0">
        <tr class="tabel_header">
          <th colspan="2" >
     <?php
while($sql2=mysql_fetch_array($not)){
	  ?>
          <th colspan="4"><?php echo $sql2[project_code] . " - " . $sql2[customer];?>            <?php } ?></th>
        </tr>
        <tr class="tabel_header">
          ...
          <th width='126' class="tabel_header">Manufacturing Progress</th>
          <th width='115' class="tabel_header">Manpower Allocation</th>
        </tr>
        <?php
while($sql1=mysql_fetch_array($sql)){
	  ?>
        <tr class="tabel_body">
          ...
          <td><?php echo $sql1[projectcode] . " - " .$sql1[productioncode];?></td>
          <td ><a href="work_fab.php?data=<?php echo $sql1['nocrf'];?>" target="_blank"><b><?php echo $sql1[jobtitle];?></b></a></td>
