...
<table width="1100" border="0" cellspacing="0" cellpadding="0" class="buatform">
  <tr>
    <td width="1017"><p align="center">
      ...
      <p>
        <?
	 $sql=mysql_query("SELECT * FROM a_crf WHERE (a_crf.approval='Y') AND (a_crf.projectcode LIKE '%$cari%') AND (a_crf.quality=1) ");
	  $totalseluruhpendaftar=mysql_num_rows($sql);
	  if($totalseluruhpendaftar=='0'){
	  echo "<center><blink>No data available in table </bink></center>";
	  } else {
	  ...
	  ?>
      <table width="989" border="0" cellspacing="1" cellpadding="0">
        <tr class="tabel_header">
          <th width='43' >No.</th>
          <th width='98'>PROD.CODE</th>
          <th width='541'>Job Title</th>
          <th width='67'>ITP</th>
          <th width='80'>Quality Inspection</th>
          <th width='81'>Calibration</th>
          <th width='71'>MDR</th>
        </tr>
        <? 
	  while($sql1=mysql_fetch_array($sql)){
	  ?>
        <tr class="tabel_body">
          ...
          <td><?php echo $sql1[projectcode] . " - " .$sql1[productioncode];?></td>
          <td ><a href="work_qc.php?data=<?php echo $sql1[nocrf];?>" target="_blank"><b>
            <?=$sql1[jobtitle]?>
          </b></a></td>
          <td align='center'>&nbsp;</td>
          <td align='center'>&nbsp;</td>
