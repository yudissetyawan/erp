...
<tbody>  
  ...
  do {
    ...
    <tr class="tabel_body" onclick="this.style.fontWeight='bold';" ondblclick="this.style.fontWeight='normal';" title="click data for 'highlight', and Double click to remove highlight">
      ...
      <td align="center">
		<?php
		$vidbpb = $row_rsitem['id'];
		mysql_select_db($database_core, $core);
		$query_rsbpbno = "SELECT p_bpb_header.bpb_no FROM p_bpb_header WHERE p_bpb_header.id = '$vidbpb'";
		$rsbpbno = mysql_query($query_rsbpbno, $core) or die(mysql_error());
		$row_rsbpbno = mysql_fetch_assoc($rsbpbno);
		$totalRows_rsbpbno = mysql_num_rows($rsbpbno);
		
		echo $row_rsbpbno['bpb_no']; ?>
      </td>
