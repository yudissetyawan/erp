...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  ...
  $insertSQL = sprintf("INSERT INTO p_ppereq_core (ppereq_header, id_item, ppereq_qty, ppereq_unit, dateinuse, ppereq_receiver, ppereq_remark) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                       ...
                       GetSQLValueString(functyyyymmdd($_POST['dateinuse']), "text"),
                       GetSQLValueString($_POST['ppereq_receiver'], "int"),
                       GetSQLValueString($_POST['remark'], "text"));

  mysql_select_db($database_core, $core);
  $Result1 = mysql_query($insertSQL, $core) or die(mysql_error());

  $insertGoTo = "ppereq_viewdetail.php?data=$vidheader";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_core, $core);
$query_rsreceiver = "SELECT id, firstname, midlename, lastname FROM h_employee WHERE code = 'K' ORDER BY firstname ASC";
$rsreceiver = mysql_query($query_rsreceiver, $core) or die(mysql_error());
$row_rsreceiver = mysql_fetch_assoc($rsreceiver);
$totalRows_rsreceiver = mysql_num_rows($rsreceiver);
?>
...
<title>Entry PPE Request Detail</title>
...
function showItem(sid) {
	...
	xmlhttp.open("GET","ppereq_getitem.php?q=" + sid + "&p=" + r, true);
	xmlhttp.send();
}
</script>
...
<b>Add Item PPE Request</b><br /><br />
...
<form action="<?php echo $editFormAction; ?>" method="POST" name="form1" class="General" id="form1">
  <table width="700" border="0" class="General">
    ...
    <tr>
      ...
      <td>:</td>
      <td><input type="text" name="dateinuse" id="tanggal8" size="12" /></td>
    </tr>
    <tr>
      <td class="General">Receiver</td>
      <td>:</td>
      <td><select name="ppereq_receiver" id="ppereq_receiver" class="required" title="Please select Receiver">
        <option value="">-- Select Receiver --</option>
        <?php
do {  
?>
        <option value="<?php echo $row_rsreceiver['id']?>"><?php echo $row_rsreceiver['firstname']?> <?php echo $row_rsreceiver['midlename']; ?> <?php echo $row_rsreceiver['lastname']; ?></option>
        <?php
} while ($row_rsreceiver = mysql_fetch_assoc($rsreceiver));
  $rows = mysql_num_rows($rsreceiver);
  if($rows > 0) {
      mysql_data_seek($rsreceiver, 0);
	  $row_rsreceiver = mysql_fetch_assoc($rsreceiver);
  }
?>
      </select></td>
    ...
    <tr>
      ...
      <td>:</td>
      <td><textarea name="remark" id="remark" cols="40" rows="3" class="required" title="Remark is required"></textarea></td>
    </tr>
    <tr>
      <td class="General">&nbsp;</td>
      <td>&nbsp;</td>
    ...
    <tr>
      <td class="General">&nbsp;</td>
      <td class="General">&nbsp;</td>
      <td class="General"><input type="submit" name="submit" id="submit" value="Add to PPE Request" onClick="return periksa();" />
</td>
    </tr>
  </table>
