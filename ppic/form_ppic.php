...
$query_Recordset1 = sprintf("SELECT * FROM a_proj_code WHERE complete = %s", GetSQLValueString($colname_Recordset1, "text"));
...
?>
      ...
      <p>
        <?
	  $sql=mysql_query("SELECT * FROM a_crf  WHERE (a_crf.approval='Y') AND (a_crf.projectcode LIKE '%$cari%') ");
	  ...
	  ?>
        <tr class="tabel_body">
          ...
          <td ><a href="#"><b onClick="MM_openBrWindow('../prj/viewcrfdetail.php?data=<?php echo $sql1[nocrf];?>','crf','width=1200,height=500')">
            <?=$sql1[jobtitle]?>
</b></a></td>
             <td align='center'><?php 
		if ($sql1[marketing]==1) 
  ...
  <tr>
    <td width="950">
      ...
      <p align="center"><strong>Find Result - Work Status </strong></p>
		...
		$sql=mysql_query("SELECT * FROM a_crf WHERE (a_crf.approval='Y') AND (a_crf.projectcode LIKE '%$cari%') AND (a_crf.production=1) ");
	  $totalseluruhpendaftar=mysql_num_rows($sql);
	  if($totalseluruhpendaftar=='0'){
	 echo "<center><blink>No data available in table </bink></center>";
      ...
      <table width="1032" border="0" cellspacing="1" cellpadding="0">
        <tr class="tabel_header">
          <th colspan="2" >
            <?
while($sql2=mysql_fetch_array($not)){
	  ?>
        ...
        <tr class="tabel_header">
          <th width="23" class="tabel_header">No.</th>
          <th width='89' class="tabel_header">Production Code</th>
          <th width='489' class="tabel_header">Job Title</th>
          <th width='88' class="tabel_header">&nbsp;</th>
          <th width='61' class="tabel_header">Planning</th>
          <th width='105' class="tabel_header">Environmental</th>
          <th width='62' class="tabel_header">Preventive</th>
          <th width='104' class="tabel_header">Currative</th>
        </tr>
        <?
while($sql1=mysql_fetch_array($sql)){
	  ?>
        <tr class="tabel_body">
          ...
          <td><?php echo $sql1[projectcode] . " - " .$sql1[productioncode];?></td>
          <td ><a href="work_ppic.php?data=<?php echo $sql1['nocrf'];?>" target="_blank"><b><?php echo $sql1[jobtitle];?></b></a></td>
          <td align="center"><a href="#" onClick="MM_openBrWindow('hasil_kerja.php?data=<?php echo $sql1['nocrf'];?>','','scrollbars=yes,resizable=yes,width=700,height=500')">View Attachment</a></td>
            <td ></td>
            <td></td>
            <td >&nbsp;</td>
            <td >&nbsp;</td>
          <td width="0"></b><td width="0"></a></tr>
        <?
		}
		?>
      </table>
