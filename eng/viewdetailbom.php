...
mysql_select_db($database_core, $core);
$query_Recordset2 = sprintf("SELECT e_drawing_file.drawingno, maker.firstname AS makername, a_customer.customername, e_header_bom.status, e_header_bom.location, e_header_bom.tw, e_header_bom.tlgh, a_production_code.productioncode, a_proj_code.project_code, e_header_bom.revision, e_header_bom.`date`, checked.firstname AS checkedby, approve.firstname AS approvedby FROM e_header_bom LEFT JOIN e_drawing_file ON e_header_bom.drawingno=e_drawing_file.id LEFT JOIN  h_employee AS maker ON e_header_bom.createdby=maker.id LEFT JOIN a_customer ON e_header_bom.customer=a_customer.id LEFT JOIN a_production_code ON e_header_bom.productioncode=a_production_code.productioncode LEFT JOIN a_proj_code ON e_header_bom.projectcode=a_proj_code.project_code LEFT JOIN  h_employee AS checked ON e_header_bom.checkedby=checked.id LEFT JOIN h_employee AS approve ON e_header_bom.approvedby = approve.id WHERE e_header_bom.id = %s", GetSQLValueString($colname_Recordset2, "int"));
...
<content>
  ...
  </div><?php if($row_Recordset2['status']=='Not Active') { ?>
  <input id="revisibutton" type="button" value="revisi" onclick="window.location.href='editbom.php?data=<?php echo $_GET['data'] ?>'"/>
  <?php } else { ?>
  <p>&nbsp;</p>
  <p>&nbsp;</p>
  <p>&nbsp;</p>
  ...
  <div id="content">
    ...
    <table width="1373" class="tableclass1">
       ...
       <tr class="content1">
         <td></td>
         <td colspan="9"><?php echo $row_Recordset1['materialname']; ?><script> var totalweight<?php echo $row_Recordset1['id']; ?>=0; </script></td>
         <?php
...
FROM e_core_bom a WHERE a.headercorebom = %s", GetSQLValueString($row_Recordset1['id'], "text"));
	...
	?>
        <form name="form<?php echo $n."-".$i ; ?>" id="form<?php echo $n."-".$i ; ?>" >
          <tr class="content2" onkeyup="calt<?php echo $Rows['id'];?>()" bgcolor="<?php if(!isset($k))$k=0; if($k=='win') echo "#D9EDE0"; else echo "DFEDCB"; $k++ ?>">
            ...
            <td align="center"><?php echo formatFloat($Rows['spec']);?></td>
            ...
            <td align="right"><?php echo formatFloat($Rows['paintarea']); if(!isset($paTotal))$paTotal=0; $paTotal=$paTotal + $Rows['paintarea']; ?></td>
            ...
            <td align="center"><div style=" font width:49px; max-width:49px;" class="edit" id="<?php echo $Rows['id']; ?>-remark"><?php if(!isset($Rows['remark']) || ($Rows['remark'] == ""))echo "    "; else { echo $Rows['remark'];};?></div></td>
          </tr>
        </form>
        ...
        <tr bgcolor="#FFFFFF">
          	<td>&nbsp;</td>
          	<td>&nbsp;</td>
          	<td colspan="2"><div align="center"><?php echo $row_Recordset1['spec']; ?></div></td>
          	<td><div align="center"><?php echo $row_Recordset1['spec2']; ?></div></td>
          	<td><div align="center"><?php echo $row_Recordset1['spec3']; ?></div></td>
          	<td>&nbsp;</td>
          	<td>&nbsp;</td>
          	<td>&nbsp;</td>
          	<td>&nbsp;</td>
